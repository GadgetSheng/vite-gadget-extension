...
| 多环境切换 | 多条规则对应不同 URL 片段，只开其中几条模拟「部分可用」。 |
...
|------|------|
| **Label** | 展示用别名，便于列表识别。 |
| **URL** | **子串包含（忽略大小写）**；或以 **`re:`** 开头写正则表达式（强制 `i`）；或使用 **`/pattern/flags`**（flags 仅 `gimsuy`，且自动保证含 `i`）。均针对解析后的**绝对 URL 字符串**。宜避免过短子串误伤。 |
...
1. **MVP**：单规则、**URL 子串 / 正则**（忽略大小写）、Method 精确、Mock 响应 + 全局开关 + 列表规则开关。
...
| 日期 | 议题 | 结论 |
|------|------|------|
| 2025-03-25 | 问题 1：URL 匹配方式 | ~~前缀 / 子串~~ **当前**：**忽略大小写子串** + **`re:` 正则** + **`/pattern/flags`**；字段键名仍为 `urlPrefix`。 |
